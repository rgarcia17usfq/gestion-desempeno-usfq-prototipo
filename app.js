...
const STORAGE='usfq-gd-v040-state';
...
function unifiedResultHtml(model){
  const s1=model.overall;
  const s2=92;
  const delta=s2-s1;
  return `<div class="result-overview-grid">
    ${resultSummaryHtml(model,'Resultado del período · Evaluación 1')}
    <section class="panel result-plan-panel"><div class="panel-heading"><div><h2>Plan del período</h2><p>Acciones acordadas después del resultado.</p></div>${status('En seguimiento','neutral')}</div><ul class="clean-list"><li>Fortalecer automatización de reportes recurrentes.</li><li>Completar capacitación en gestión de indicadores.</li><li>Revisar avance en el siguiente check-in.</li></ul><button class="btn secondary open-calculation">Ver cómo se calcula</button></section>
  </div>
  <section class="panel annual-summary-panel">
    <div class="panel-heading"><div><p class="section-kicker">Evolución anual</p><h2>Evaluación 1 vs Evaluación 2</h2><p>Comparación de los dos resultados del ciclo, sin asumir una fórmula anual no validada.</p></div><span class="delta-badge ${delta>=0?'positive':'negative'}">${delta>=0?'+':''}${delta.toFixed(1)} pts</span></div>
    <div class="annual-score-strip"><div><span>Evaluación 1</span><strong>${s1.toFixed(1)}</strong><small>/100 · Enero 2028</small></div><div class="annual-score-arrow">→</div><div><span>Evaluación 2</span><strong>${s2.toFixed(1)}</strong><small>/100 · Junio 2028</small></div></div>
    <div class="compare-bars compact-compare"><div class="compare-row"><strong>Evaluación 1</strong><div class="compare-track"><span style="width:${Math.round(s1)}%"></span></div><strong>${s1.toFixed(1)}</strong></div><div class="compare-row"><strong>Evaluación 2</strong><div class="compare-track"><span style="width:${s2}%"></span></div><strong>${s2.toFixed(1)}</strong></div></div>
    <div class="info-callout"><strong>Resultado anual:</strong> la vista compara ambos períodos. La fórmula para un eventual resultado anual consolidado permanece pendiente de definición por RRHH.</div>
  </section>
  <section class="panel talent-unified-panel">
    ...
    <div class="summary-grid result-cycle-metrics"><div class="summary-box"><span class="label">Objetivos</span><strong>2</strong><small>KPIs del ciclo</small></div><div class="summary-box"><span class="label">Sesiones</span><strong>3</strong><small>Actas registradas</small></div><div class="summary-box"><span class="label">Resultado E1</span><strong>${Math.round(s1)}%</strong><small>Primer período</small></div><div class="summary-box"><span class="label">Resultado E2</span><strong>${s2}%</strong><small>Cierre del ciclo</small></div></div>
    <div class="timeline-vertical result-timeline"><div class="timeline-item"><div class="date">Agosto 2027</div><div class="vline"></div><div><h4>Sesión de arranque</h4><p>Perfil revisado, objetivos/KPIs y acuerdos definidos.</p></div></div><div class="timeline-item"><div class="date">Octubre 2027</div><div class="vline"></div><div><h4>Check-in 1</h4><p>Avances, obstáculos y acuerdos documentados.</p></div></div><div class="timeline-item"><div class="date">Enero 2028</div><div class="vline"></div><div><h4>Evaluación 1 · ${s1.toFixed(1)}/100</h4><p>Resultado del primer período y plan de trabajo.</p></div></div><div class="timeline-item"><div class="date">Abril 2028</div><div class="vline"></div><div><h4>Check-in 2</h4><p>Seguimiento con referencia al resultado del primer período.</p></div></div><div class="timeline-item"><div class="date">Junio 2028</div><div class="vline"></div><div><h4>Evaluación 2 · ${s2.toFixed(1)}/100</h4><p>Cierre del ciclo y consolidación del perfil de talento.</p></div></div></div>
  </section>`;
}

function renderResults(){
  const c=$('#resultsContent');const model=resultModel('own');
  ...
  <div id="myResult" class="results-pane active">${unifiedResultHtml(model)}</div>
