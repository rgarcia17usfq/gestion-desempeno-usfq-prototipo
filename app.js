...
const STORAGE='usfq-gd-v050-state';
...
function applyRole(){
  ...
  if(r==='admin' && !['admin','dashboard','processGuide'].includes(state.page)) state.page='admin';
...
function nextStageCopy(role){
  ...
  const map={
    colaborador:{start:['Revisa tu perfil y participa en la sesión de inicio','Consulta tu perfil, funciones y objetivos que se acordarán con tu jefatura.','profile'],check1:['Prepárate para el Check-in 1','Revisa tus objetivos y avances antes de la conversación de seguimiento.','kpis'],eval1:['Completa tus evaluaciones','Realiza tu autoevaluación y la evaluación a tu jefe.','evaluations'],check2:['Revisa tus avances del segundo período','Consulta tus objetivos y los acuerdos anteriores antes del Check-in 2.','sessions'],eval2:['Completa la Evaluación 2','Realiza las evaluaciones habilitadas para el cierre del ciclo.','evaluations']},
    jefe:{start:['Realiza las sesiones de inicio de tu equipo','Las sesiones se registran desde la pestaña Sesiones.','sessions'],check1:['Realiza el Check-in 1 de tu equipo','Registra avances, obstáculos, acuerdos y próximos pasos.','sessions'],eval1:['Evalúa a tu equipo','En Evaluaciones se habilita cada colaborador cuando complete sus pasos previos.','evaluations'],check2:['Realiza el Check-in 2 de tu equipo','Da seguimiento a objetivos, resultados del primer período y acuerdos.','sessions'],eval2:['Completa la Evaluación 2 de tu equipo','En Evaluaciones se muestran las personas listas para la segunda evaluación.','evaluations']}
  };
...
function renderDashboard(){
  ...
  if(state.role==='admin'){
    ...
    <div class="cards-grid three"><article class="metric-card activity-card"><div class="metric-icon">1</div><div><span class="metric-label">Próxima acción</span><h3>${state.campaignStarted?'Monitorear pendientes':'Validar e iniciar ciclo'}</h3><p>${state.campaignStarted?'Revisa el cumplimiento de sesiones y evaluaciones.':'Completa calendario, matriz y comunicaciones antes del inicio.'}</p><button class="text-action" data-go="admin">Abrir administración →</button></div></article><article class="metric-card"><div class="metric-icon">GP</div><div><span class="metric-label">Población y estructura</span><h3>Nómina GP</h3><p>Personas, cargos, áreas y jefaturas se sincronizan desde la fuente institucional.</p></div></article><article class="metric-card"><div class="metric-icon">♢</div><div><span class="metric-label">Recordatorios</span><h3>3 reglas</h3><p>Inicio, pendiente y vencimiento.</p></div></article></div>`;
  }else if(state.role==='comite'){
    renderCommittee();return;
  }else{
    const isBoss=state.role==='jefe'; const copy=nextStageCopy(state.role);
    c.innerHTML=`<div class="dashboard-hero page-heading"><div><p class="eyebrow">Ciclo de evaluación ${state.cycle==='annual'?'2027-2028':'Piloto 2027'}</p><h1>Hola, Ricardo</h1><p class="lead">${isBoss?'Gestiona tu propio ciclo de evaluación y da seguimiento al proceso de tu equipo.':'Gestiona tu ciclo de evaluación y consulta tus actividades, acuerdos y resultados.'}</p></div>${status('Ciclo activo','success')}</div>
    ...
    <div class="cycle-card panel"><div class="panel-heading"><div><h2>Mi ciclo de evaluación</h2><p>${state.cycle==='annual'?'Agosto 2027 - Junio 2028':'Enero - Julio 2027'}</p></div><div class="progress-summary"><strong>${Math.round((stageIndex()+1)/5*100)}%</strong><span>etapa simulada</span></div></div>${timelineHtml(state.stage)}</div>
    ...
    ${isBoss?`<section class="panel"><div class="panel-heading"><div><h2>Estado de mi equipo</h2><p>Vista de seguimiento. Las evaluaciones se realizan exclusivamente desde la pestaña Evaluaciones.</p></div></div><div class="mini-checklist"><div class="mini-check"><span>Ana Torres</span>${status(teamStageLabel(state.team[0]),'success')}</div><div class="mini-check"><span>Diego Vega</span>${status(teamStageLabel(state.team[1]),'success')}</div><div class="mini-check"><span>Miguel León</span>${status(teamStageLabel(state.team[3]),'warning')}</div></div><button class="btn secondary" data-go="team">Ver avance del equipo</button></section>`:''}`;
  }
...
function renderTeam(){
  ...
  $('#teamMetrics').innerHTML=`<article><span>${state.team.length}</span><p>Colaboradores</p></article><article><span>${state.team.filter(m=>m.start).length}</span><p>Sesiones de inicio completadas</p></article><article><span>${state.team.filter(m=>memberProgress(m)>=40&&memberProgress(m)<100).length}</span><p>En proceso</p></article><article><span>${state.team.filter(m=>memberProgress(m)===100).length}</span><p>Ciclos completados</p></article>`;
  $('#teamRows').innerHTML=relevant.map(m=>`<tr><td><strong>${m.name}</strong><small>${m.area}</small></td><td>${m.role}</td><td><div class="team-progress-cell"><div class="progress compact"><div style="width:${memberProgress(m)}%"></div></div><strong>${memberProgress(m)}%</strong></div><small>${m.start?'Sesión de inicio ✓':'Sesión de inicio pendiente'} · ${m.check1?'Check-in 1 ✓':'Check-in 1 pendiente'}</small></td><td>${m.self?'Autoevaluación ✓':'Autoevaluación pendiente'}<small>${m.upward?'Evaluación a jefe ✓':'Evaluación a jefe pendiente'}</small></td><td>${teamStageLabel(m)}</td><td class="team-status">${teamStatusPill(m)}</td><td><button class="btn small secondary open-member" data-member="${m.id}">Ver avance</button></td></tr>`).join('');
...
function renderTeamMember(){
  const m=member(state.selectedMember)||state.team[0];
  $('#teamMemberContent').innerHTML=`<button class="link-btn back-team">← Volver a Mi equipo</button><div class="team-member-header"><div class="team-member-title"><div class="mini-avatar">${m.initials}</div><div><p class="eyebrow">Estado del ciclo</p><h1>${m.name}</h1><p>${m.role} · ${m.area}</p></div></div>${teamStatusPill(m)}</div>
  <section class="panel"><div class="panel-heading"><div><h2>Avance del proceso</h2><p>Esta vista es únicamente de seguimiento; las acciones se realizan desde Sesiones o Evaluaciones.</p></div><strong class="member-progress-number">${memberProgress(m)}%</strong></div><div class="progress"><div style="width:${memberProgress(m)}%"></div></div><div class="process-strip status-only"><div class="process-chip ${m.start?'done':'current'}">1. Sesión de inicio<br><small>${m.start?'Completada':'Pendiente'}</small></div><div class="process-chip ${m.check1?'done':m.start?'current':'blocked'}">2. Check-in 1<br><small>${m.check1?'Completado':'Pendiente'}</small></div><div class="process-chip ${m.s1!=null?'done':m.self&&m.upward?'current':'blocked'}">3. Evaluación 1<br><small>${m.s1!=null?'Completada':m.self&&m.upward?'Lista':'Pendiente'}</small></div><div class="process-chip ${m.check2?'done':m.s1!=null?'current':'blocked'}">4. Check-in 2<br><small>${m.check2?'Completado':'Pendiente'}</small></div><div class="process-chip ${m.s2!=null?'done':m.check2?'current':'blocked'}">5. Evaluación 2<br><small>${m.s2!=null?'Completada':'Pendiente'}</small></div></div></section>
  <div class="two-col" style="margin-top:18px"><section class="panel"><div class="panel-heading"><div><h2>Perfil y objetivos</h2><p>Información de referencia para seguimiento.</p></div>${m.profile?status('Perfil disponible','success'):status('Perfil pendiente','warning')}</div><div class="mini-checklist"><div class="mini-check"><span>Cargo</span><strong>${m.role}</strong></div><div class="mini-check"><span>Objetivos / KPIs</span><strong>${m.kpis}</strong></div><div class="mini-check"><span>Evaluación de servicio</span><strong>${m.serviceEligible?'Aplica':'No aplica'}</strong></div></div></section><section class="panel"><div class="panel-heading"><div><h2>Estado de evaluaciones</h2><p>Resumen de disponibilidad y cierre.</p></div></div><div class="mini-checklist"><div class="mini-check"><span>Autoevaluación</span>${m.self?status('Completada','success'):status('Pendiente','warning')}</div><div class="mini-check"><span>Evaluación a jefe</span>${m.upward?status('Completada','success'):status('Pendiente','warning')}</div><div class="mini-check"><span>Evaluación 1</span>${m.s1!=null?status('Completada','success'):status('Pendiente','neutral')}</div><div class="mini-check"><span>Evaluación 2</span>${m.s2!=null?status('Completada','success'):status('Pendiente','neutral')}</div></div></section></div>`;
...
function openSessionBuilder(type,targetId){
  const m=targetId?member(targetId):{name:'Ricardo García Ortiz',role:'Coordinador de Procesos y Automatización',profile:true};const isStart=type==='Sesión de inicio';
  ...
  const html=`<div class="meeting-builder">${isStart?`<div class="meeting-section"><h3>1. Perfil y contexto</h3><div class="read-only-block"><h4>${m.name}</h4><p>${m.role} · Perfil de cargo ${targetId&&m.profile===false?'no disponible':'disponible para revisión'}.</p></div></div><div class="meeting-section"><h3>2. Objetivos y KPIs construidos en conjunto</h3><p class="helper">Registra cada objetivo con indicador, línea base y meta.</p><div class="objective-edit"><input class="control" value="Reducir tiempo de respuesta de solicitudes"/><input class="control" value="Tiempo promedio de respuesta"/><input class="control" value="48 horas"/><input class="control" value="24 horas"/></div><div class="objective-edit"><input class="control" value="Digitalizar expedientes administrativos"/><input class="control" value="% de expedientes digitalizados"/><input class="control" value="35%"/><input class="control" value="100%"/></div></div><div class="meeting-section"><h3>3. Observaciones generales</h3><textarea class="control" placeholder="Aspectos relevantes conversados al inicio del ciclo..."></textarea></div>`:`<div class="meeting-section"><h3>1. Contexto desde la conversación anterior</h3><p class="helper">Resume los principales temas revisados desde el último check-in: cambios relevantes, compromisos previos o situaciones que dan contexto a esta conversación.</p><textarea class="control" placeholder="Ej.: se revisaron los acuerdos anteriores, cambios de prioridad y avances que requieren seguimiento..."></textarea></div><div class="meeting-section"><h3>2. Seguimiento de objetivos</h3>${objectiveRows}</div><div class="meeting-section"><h3>3. Avances</h3><textarea class="control" placeholder="Principales avances desde la conversación anterior..."></textarea></div><div class="meeting-section"><h3>4. Obstáculos / ajustes</h3><textarea class="control" placeholder="Obstáculos, dependencias o ajustes requeridos..."></textarea></div>`}<div class="meeting-section"><h3>${isStart?'4':'5'}. Acuerdos</h3><textarea class="control" placeholder="Acuerdos concretos..."></textarea></div><div class="meeting-section"><h3>${isStart?'5':'6'}. Próximos pasos</h3><textarea class="control" placeholder="Próximos pasos, responsables o fechas..."></textarea></div></div><div class="modal-actions"><button class="btn secondary" id="cancelSession">Cancelar</button><button class="btn primary" id="saveSession" data-session-type="${type}" data-session-target="${targetId||''}">Guardar acta</button></div>`;
...
function unifiedResultHtml(model,model2){
  ...
  <section class="panel talent-unified-panel">
    ...
    <div class="talent-executive-grid"><div class="read-only-block"><h4>Funciones generales</h4><ul class="clean-list"><li>Coordinar iniciativas de mejora continua y automatización.</li><li>Levantar y rediseñar procesos institucionales.</li><li>Dar seguimiento a proyectos e indicadores de mejora.</li></ul></div><div class="read-only-block"><h4>Objetivos del ciclo</h4><ul class="clean-list"><li>Reducir tiempo de respuesta de solicitudes.</li><li>Digitalizar expedientes administrativos.</li></ul></div></div><div class="summary-grid result-cycle-metrics"><div class="summary-box"><span class="label">Resultado E1</span><strong>${s1.toFixed(1)}%</strong><small>Primer período</small></div><div class="summary-box"><span class="label">Resultado E2</span><strong>${s2.toFixed(1)}%</strong><small>Cierre del ciclo</small></div></div>
    <div class="timeline-vertical result-timeline">${resultTimelineItem('Agosto 2027','start','Sesión de inicio','Perfil revisado, objetivos/KPIs y acuerdos definidos.')}${resultTimelineItem('Octubre 2027','check1','Check-in 1','Avances, obstáculos y acuerdos documentados.')}${resultTimelineItem('Enero 2028','eval1',`Evaluación 1 · ${s1.toFixed(1)}/100`,'Resultado del primer período y plan de trabajo.')}${resultTimelineItem('Abril 2028','check2','Check-in 2','Seguimiento con referencia al resultado del primer período.')}${resultTimelineItem('Junio 2028','eval2',`Evaluación 2 · ${s2.toFixed(1)}/100`,'Cierre del ciclo y consolidación del perfil de talento.')}</div>
  </section>`;
}
...
function renderAdmin(){
  ...
  renderNotificationRules();renderExceptions();renderQuestionAdmin();renderWeightSchemes();renderMatrix();
}
function renderMatrix(){
  const body=$('#matrixRows');if(!body)return;
  body.innerHTML=state.team.map(m=>`<tr><td><strong>${m.name}</strong><small>${m.role} · ${m.area}</small></td><td>Ricardo García</td><td>✓</td><td>✓</td><td>0</td><td><label class="service-toggle-cell"><input class="service-checkbox service-edit" data-member="${m.id}" type="checkbox" ${m.serviceEligible?'checked':''}><span>${m.serviceEligible?'Aplica':'No aplica'}</span></label>${m.serviceEligible&&m.serviceScore?`<small>Nota servicio: ${m.serviceScore}/100 · fuente externa</small>`:''}</td><td>${m.profile?status('Correcto','success'):status('Sin perfil de cargo','warning')}</td></tr>`).join('')+`<tr><td><strong>Ricardo García Ortiz</strong><small>Coordinador · Mejoramiento Continuo</small></td><td>María Andrade</td><td>✓</td><td>✓</td><td>${state.team.length}</td><td><span class="small-muted">No aplica</span></td><td>${status('Jefe también evaluado','success')}</td></tr>`;
...
const processGuideData={
  ...
  eval2:{title:'5. Evaluación 2 y cierre',purpose:'Cerrar el ciclo, comparar resultados y habilitar el análisis del Comité de Talento.',colaborador:['Completa las evaluaciones habilitadas.','Consulta resultados generales y recorrido del ciclo.'],jefe:['Evalúa desde la pestaña Evaluaciones.','Consulta avance del equipo desde Mi equipo.','Consulta resultados generales autorizados.'],admin:['Cierra campaña y valida integridad de resultados.','Mantiene reglas de servicio, instrumentos y ponderaciones.'],comite:['Filtra resultados por área, persona y rango.','Abre perfiles de talento y actas autorizadas.','Registra planes de acción o decisiones.']}
};
...
$('#teamSearch').addEventListener('input',renderTeam);$('#teamStatusFilter').addEventListener('change',renderTeam);
...
document.addEventListener('click',e=>{
  ...
  if(e.target?.id==='saveSession'){const target=e.target.dataset.sessionTarget,type=e.target.dataset.sessionType;if(target){const m=member(target);if(type==='Sesión de inicio'){m.start=true;m.kpis=Math.max(m.kpis,2)}else if(type==='Check-in 2')m.check2=true;else m.check1=true;}else{state.sessions.push({id:Date.now(),date:new Date().toLocaleDateString('es-EC'),type,summary:type==='Sesión de inicio'?'Perfil, objetivos/KPIs y acuerdos.':'Avances, obstáculos y acuerdos.',status:'Completada',participants:'Ricardo / María',agreements:['Acuerdo registrado desde el prototipo.'],next:['Próximo paso registrado desde el prototipo.']});}saveState();closeModal();toast('Acta guardada');if(target){renderTeamMember()}else renderSessions();}
