...
const STORAGE='usfq-gd-v070-state';
...
const defaultState={
  role:'jefe',cycle:'pilot',stage:'eval1',page:'dashboard',campaignStarted:false,selectedMember:'ana',selectedArea:'Mejoramiento Continuo',instrumentType:'manager',
  matrixFilters:{person:'',manager:'all'},trackingFilters:{person:'',status:'all',stage:'all'},
  ...
  sessions:[
    {id:1,date:'15/08/2027',type:'Sesión de inicio',summary:'Perfil, acuerdos y próximos pasos.',status:'Completada',participants:'Ricardo / María',general:'Se revisó el perfil de cargo y se acordaron prioridades del ciclo.',agreements:['Priorizar automatización de solicitudes repetitivas.','Medir semanalmente el tiempo promedio de respuesta.'],next:['Revisar compromisos en la primera reunión de feedback.']},
    {id:2,date:'18/10/2027',type:'Reunión de feedback 1',summary:'Contexto, obstáculos y acuerdos / próximos pasos.',status:'Completada',participants:'Ricardo / María',advances:'Se automatizó el primer grupo de solicitudes y mejoró el tiempo de respuesta.',obstacles:'Acceso parcial a datos históricos.',agreements:['Completar acceso a datos antes de noviembre.'],next:['Validar tendencia del indicador al cierre de diciembre.']}
  ],
  team:[
    {id:'ana',name:'Ana Torres',initials:'AT',role:'Analista de Procesos',area:'Mejoramiento Continuo',profile:true,start:true,check1:true,check2:true,self:true,upward:true,manager:false,s1:93,s2:null,kpis:2,plan:'Pendiente',hasSubordinates:true,serviceEligible:false,resultComponents:{e1:{self:96,manager:92,upward:94}}},
    {id:'diego',name:'Diego Vega',initials:'DV',role:'Analista de Datos',area:'Mejoramiento Continuo',profile:true,start:true,check1:true,check2:true,self:true,upward:true,manager:true,s1:88,s2:92,kpis:2,plan:'Pendiente',hasSubordinates:true,serviceEligible:true,serviceScore:94,resultComponents:{e1:{self:90,manager:86,upward:85.4286,service:94},e2:{self:94,manager:92,upward:90.2857,service:94}}},
    {id:'carolina',name:'Carolina Paz',initials:'CP',role:'Asistente de Proyecto',area:'Proyectos',profile:false,start:true,check1:true,check2:true,self:true,upward:true,manager:true,s1:95,s2:91,kpis:2,plan:'En seguimiento',hasSubordinates:true,serviceEligible:false,resultComponents:{e1:{self:100,manager:95,upward:94.2857},e2:{self:95,manager:91,upward:90.4286}}},
    {id:'miguel',name:'Miguel León',initials:'ML',role:'Coordinador de Datos',area:'Analítica',profile:true,start:false,check1:false,check2:false,self:false,upward:false,manager:false,s1:null,s2:null,kpis:0,plan:'Pendiente',hasSubordinates:true,serviceEligible:true,serviceScore:91,resultComponents:{}}
  ],
  evalAnswers:{},
...
const annualSteps=[
  ...
  {key:'check1',month:'Octubre',label:'Reunión de feedback 1'},
  {key:'eval1',month:'Enero',label:'Evaluación 1'},
  {key:'check2',month:'Abril',label:'Reunión de feedback 2'},
  {key:'eval2',month:'Junio',label:'Evaluación 2'}
];
const pilotSteps=[
  {key:'start',month:'Enero',label:'Sesión de inicio'},
  {key:'check1',month:'Marzo',label:'Reunión de feedback 1'},
  {key:'eval1',month:'Abril',label:'Evaluación 1'},
  {key:'check2',month:'Mayo',label:'Reunión de feedback 2'},
  {key:'eval2',month:'Julio',label:'Evaluación 2'}
];
...
function navigate(page){
  ...
  if(page==='processGuide')renderProcessGuide();
  setDrawer(false);
  window.scrollTo({top:0,behavior:'smooth'});
}

function stageLabel(key=state.stage){return ({start:'Sesión de inicio',check1:'Reunión de feedback 1',eval1:'Evaluación 1',check2:'Reunión de feedback 2',eval2:'Evaluación 2 y cierre'})[key]||'Evaluación 2 y cierre'}
...
function applyRole(){
  ...
  if(r==='decano'){ $$('.role-person,.role-jefe,.role-jefe-inline').forEach(x=>x.style.display=''); const resultNav=$('.nav-link[data-page="results"]'); if(resultNav)resultNav.style.display='none'; }
  if(r==='admin') $$('.role-admin').forEach(x=>x.style.display='');
  if(r==='comite' && state.cycle!=='pilot') $$('.role-comite').forEach(x=>x.style.display='');
  const roleLabel={colaborador:'Colaborador',jefe:'Jefe / Evaluador',decano:'Decano / Evaluador',admin:'RRHH / Administrador',comite:'Comité de Talento'}[r];
  ...
  const comiteOpt=$('#roleSelect option[value="comite"]'); if(comiteOpt){comiteOpt.disabled=state.cycle==='pilot';comiteOpt.textContent=state.cycle==='pilot'?'Comité de Talento (post-piloto)':'Comité de Talento';}
  renderNotifications();
  const dashboardNav=$('.nav-link[data-page="dashboard"]'); if(dashboardNav) dashboardNav.style.display=(r==='admin'||r==='comite')?'none':'';
  const kpiNav=$('.nav-link[data-page="kpis"]'); if(kpiNav && (state.cycle==='pilot'||r==='decano')) kpiNav.style.display='none';
  if(r==='admin') state.page='admin';
  if(r==='comite' && state.cycle==='pilot'){state.role='admin';$('#roleSelect').value='admin';state.page='admin';return applyRole();}
  if(r==='comite' && state.page!=='processGuide') state.page='committee';
  if((r==='colaborador'||r==='jefe'||r==='decano') && ['admin','committee'].includes(state.page)) state.page='dashboard';
  if(r==='decano' && state.page==='results')state.page='team';
  if(state.cycle==='pilot' && (state.page==='kpis'||state.page==='committee')) state.page=r==='admin'?'admin':'dashboard';
  // Excepciones y Comité quedan fuera del piloto.
  $$('.post-pilot-tab,.post-pilot-pane').forEach(x=>x.style.display=state.cycle==='pilot'?'none':'');
...
function renderNotifications(){
  ...
  const byRole={
    colaborador:[{title:'Completa tus evaluaciones',body:'Autoevaluación y evaluación a tu jefe disponibles en la etapa de evaluación.',when:'Pendiente'},{title:'Reunión de feedback',body:'Consulta los acuerdos de tu última conversación cuando estén disponibles.',when:'Seguimiento'}],
    jefe:[{title:'Evalúa a tu equipo',body:'Ana Torres está lista para que completes su evaluación.',when:'Pendiente'},{title:'Seguimiento del equipo',body:'Consulta Evaluaciones para identificar quién aún no está listo.',when:'Seguimiento'}],
    decano:[{title:'Evalúa a tu equipo',body:'Tu rol participa como evaluador del equipo administrativo; no realiza autoevaluación.',when:'Pendiente'}],
    admin:[{title:'Seguimiento del proceso',body:'Usa Seguimiento para identificar personas por estado y etapa.',when:'Operación'},{title:'Validación de matriz',body:'Revisa población administrativa y relaciones jefe-evaluado antes de iniciar el ciclo.',when:'Antes del inicio'}],
    comite:[{title:'Módulo post-piloto',body:'El Comité de Talento queda fuera del alcance del piloto.',when:'Evolución'}]
  };
...
function nextStageCopy(role){
  const stage=state.stage; const pilot=state.cycle==='pilot';
  const boss={start:['Realiza las sesiones de inicio de tu equipo','Las sesiones se registran desde la pestaña Sesiones.','sessions'],check1:['Realiza la Reunión de feedback 1','Registra contexto, obstáculos y acuerdos / próximos pasos.','sessions'],eval1:['Evalúa a tu equipo','En Evaluaciones se habilita cada colaborador cuando complete sus pasos previos.','evaluations'],check2:['Realiza la Reunión de feedback 2','Retoma el contexto anterior y registra obstáculos y acuerdos / próximos pasos.','sessions'],eval2:['Completa la Evaluación 2 de tu equipo','En Evaluaciones se muestran las personas listas para la segunda evaluación.','evaluations']};
  const map={
    colaborador:{start:['Revisa tu perfil y participa en la sesión de inicio','Consulta tu perfil y participa en la conversación de inicio.','profile'],check1:['Prepárate para la Reunión de feedback 1','Revisa los acuerdos anteriores antes de la conversación.','sessions'],eval1:['Completa tus evaluaciones','Realiza tu autoevaluación y la evaluación a tu jefe.','evaluations'],check2:['Prepárate para la Reunión de feedback 2','Consulta los acuerdos anteriores antes de la conversación.','sessions'],eval2:['Completa la Evaluación 2','Realiza las evaluaciones habilitadas para el cierre del ciclo.','evaluations']},
    jefe:boss,decano:boss
  };
  return (map[role]||map.colaborador)[stage]||map.colaborador.eval2;
}
function renderDashboard(){
  const c=$('#dashboardContent');
  if(state.role==='comite'){renderCommittee();return;} if(state.role==='admin'){navigate('admin');return;}
  const isBoss=['jefe','decano'].includes(state.role), isDean=state.role==='decano', copy=nextStageCopy(state.role), pilot=state.cycle==='pilot';
  const resultCard=isDean?'':`<article class="metric-card"><div class="metric-icon">%</div><div><span class="metric-label">Resultados</span><h3>${stageIndex()>=2?state.own.lastResult+'%':'Pendientes'}</h3><p>${stageIndex()>=2?'Consulta el resultado general disponible.':'Se habilitan al cerrar la evaluación.'}</p>${stageIndex()>=2?'<button class="text-action" data-go="results">Ver resultados →</button>':''}</div></article>`;
  const utilityCards=pilot?`<div class="cards-grid ${isDean?'one':'two'}"><article class="metric-card"><div class="metric-icon">▣</div><div><span class="metric-label">Última conversación</span><h3>${stageIndex()>=3?'Reunión de feedback 2':'Reunión de feedback 1'}</h3><p>Consulta las actas registradas del ciclo.</p><button class="text-action" data-go="sessions">Ver sesiones →</button></div></article>${resultCard}</div>`:`<div class="cards-grid three"><article class="metric-card"><div class="metric-icon">◎</div><div><span class="metric-label">Objetivos / KPIs</span><h3>2 objetivos</h3><p>Avance promedio: <strong>68%</strong></p><button class="text-action" data-go="kpis">Ver objetivos →</button></div></article><article class="metric-card"><div class="metric-icon">▣</div><div><span class="metric-label">Última conversación</span><h3>${stageIndex()>=3?'Reunión de feedback 2':'Reunión de feedback 1'}</h3><p>Consulta las actas registradas del ciclo.</p><button class="text-action" data-go="sessions">Ver sesiones →</button></div></article>${resultCard}</div>`;
  c.innerHTML=`<div class="dashboard-hero page-heading"><div><p class="eyebrow">Ciclo de evaluación ${pilot?'Piloto 2027':'2027-2028'}</p><h1>Hola, Ricardo</h1><p class="lead">${isDean?'Gestiona las conversaciones y evaluaciones de tu equipo administrativo. Como decano, no realizas autoevaluación.':isBoss?'Gestiona tu propio ciclo de evaluación y consulta los resultados de tu equipo.':'Gestiona tu ciclo de evaluación y consulta tus actividades, acuerdos y resultados.'}</p></div>${status('Ciclo activo','success')}</div><div class="alert-banner"><div><strong>Próxima actividad: ${copy[0]}</strong><p>${copy[1]}</p></div><button class="btn primary" data-go="${copy[2]}">Continuar</button></div><div class="cycle-card panel"><div class="panel-heading"><div><h2>${isDean?'Ciclo de evaluación del equipo':'Mi ciclo de evaluación'}</h2><p>${pilot?'Enero - Julio 2027':'Agosto 2027 - Junio 2028'}</p></div><div class="progress-summary"><strong>${Math.round((stageIndex()+1)/5*100)}%</strong><span>etapa simulada</span></div></div>${timelineHtml(state.stage)}</div>${utilityCards}${isBoss?`<section class="panel"><div class="panel-heading"><div><h2>Resultados de mi equipo</h2><p>Mi equipo consolida las calificaciones de cada colaborador; el avance operativo permanece en Evaluaciones.</p></div></div><button class="btn secondary" data-go="team">Ver resultados del equipo</button></section>`:''}`;
  $$('[data-go]',c).forEach(b=>b.addEventListener('click',()=>navigate(b.dataset.go)));
}

function renderKpis(){
  const c=$('#kpiList');c.innerHTML='';
  state.kpis.forEach(k=>{const el=document.createElement('article');el.className='panel kpi-card';el.innerHTML=`<div class="panel-heading"><div><h2>${k.title}</h2><p>${k.indicator}</p></div>${status(`${k.progress}% avance`,k.progress>=70?'success':'warning')}</div><div class="kpi-grid"><div><span class="kpi-value">Indicador<strong>${k.indicator}</strong></span></div><div><span class="kpi-value">Línea base<strong>${k.base}</strong></span></div><div><span class="kpi-value">Meta<strong>${k.meta}</strong></span></div></div><div style="margin-top:16px"><div class="goal-top"><span>Progreso registrado en reunión de feedback</span><strong>${k.progress}%</strong></div><div class="progress"><div style="width:${k.progress}%"></div></div></div>`;c.appendChild(el)});
}

function sessionStageConfig(){
  if(state.stage==='start')return {type:'Sesión de inicio',field:'start'};
  if(state.stage==='check1')return {type:'Reunión de feedback 1',field:'check1'};
  if(state.stage==='check2')return {type:'Reunión de feedback 2',field:'check2'};
  return null;
}
function renderSessions(){
  const teamBox=$('#teamSessionActions'); const isBoss=['jefe','decano'].includes(state.role);
  if(isBoss){const cfg=sessionStageConfig(); if(cfg){teamBox.innerHTML=`<section class="panel session-team-panel"><div class="panel-heading"><div><h2>${cfg.type} · Mi equipo</h2><p>Registra esta conversación desde Sesiones. Las evaluaciones se realizan exclusivamente desde Evaluaciones.</p></div>${status(stageLabel(),'neutral')}</div><div class="session-team-grid">${state.team.map(m=>{const done=!!m[cfg.field];return `<article><div><strong>${m.name}</strong><small>${m.role}</small></div>${done?status('Completada','success'):`<button class="btn small primary team-session-register" data-member="${m.id}" data-type="${cfg.type}">Registrar</button>`}</article>`}).join('')}</div></section>`;}else teamBox.innerHTML=`<div class="info-callout"><strong>Etapa actual:</strong> ${stageLabel()}. No hay una conversación obligatoria abierta en esta etapa; puedes consultar las actas registradas.</div>`;}else teamBox.innerHTML='';
  const legend=$('.session-legend'); if(legend) legend.innerHTML=state.cycle==='pilot'?'<span><i class="dot red"></i> Sesión de inicio: perfil, observaciones y acuerdos / próximos pasos</span><span><i class="dot gray"></i> Reunión de feedback: contexto, obstáculos y acuerdos / próximos pasos</span>':'<span><i class="dot red"></i> Sesión de inicio: perfil, objetivos/KPIs y acuerdos / próximos pasos</span><span><i class="dot gray"></i> Reunión de feedback: contexto, obstáculos y acuerdos / próximos pasos</span>';
  $('#sessionRows').innerHTML=state.sessions.map(s=>`<tr><td>${s.date}</td><td>${s.type.replace('Reunión de feedback','Reunión de feedback')}</td><td>${s.participants}</td><td>${state.cycle==='pilot'&&s.type==='Sesión de inicio'?'Perfil, acuerdos y próximos pasos.':s.summary}</td><td>${status(s.status,s.status==='Completada'?'success':'neutral')}</td><td><button class="link-btn view-minute" data-session="${s.id}">Ver acta</button></td></tr>`).join('')+`<tr><td>15/05/2027</td><td>Reunión de feedback 2</td><td>Ricardo / María</td><td>Contexto, obstáculos y acuerdos / próximos pasos</td><td>${stageIndex()>=3?status('Disponible','neutral'):status('Pendiente','neutral')}</td><td>${isBoss&&state.stage==='check2'?'<button class="link-btn register-own-feedback">Registrar</button>':'-'}</td></tr>`;
  $$('.view-minute').forEach(b=>b.addEventListener('click',()=>viewMinute(+b.dataset.session))); $$('.register-own-feedback').forEach(b=>b.addEventListener('click',()=>openSessionBuilder('Reunión de feedback 2',null))); $$('.team-session-register').forEach(b=>b.addEventListener('click',()=>openSessionBuilder(b.dataset.type,b.dataset.member)));
}

function viewMinute(sessionId,targetId=null,typeOverride=null){
  let data;
  if(targetId){const m=member(targetId)||committeePerson(targetId)||{name:'Colaborador'}; data={date:typeOverride==='Sesión de inicio'?'15/08/2027':'18/10/2027',type:typeOverride||'Reunión de feedback 1',participants:`${m.name} / Jefatura`,general:'Se revisó el perfil del cargo y las prioridades del período.',advances:'Se reportaron avances en los objetivos acordados.',obstacles:'Se identificó una dependencia de información externa.',agreements:['Mantener seguimiento quincenal del indicador.','Priorizar las actividades acordadas.'],next:['Revisar resultados en la siguiente conversación.']};}
...
function renderEvaluationHub(){
  const c=$('#evaluationHub'); const evaluationOpen=['eval1','eval2'].includes(state.stage); if(!evaluationOpen){c.innerHTML=`<section class="panel empty-stage"><h2>Evaluaciones no habilitadas en esta etapa</h2><p>La etapa simulada es <strong>${stageLabel()}</strong>. Cambia la etapa desde el menú del prototipo para visualizar la experiencia de Evaluación 1 o Evaluación 2.</p></section>`;return;}
  const period=state.stage==='eval2'?2:1; const isBoss=['jefe','decano'].includes(state.role); const isDean=state.role==='decano';
  if(isBoss){const rows=state.team.map(m=>{const ready=period===1?(m.self&&m.upward&&!m.manager):(m.check2&&m.s1!=null&&m.s2==null);const complete=period===1?!!m.manager:m.s2!=null;return `<tr><td><strong>${m.name}</strong><small>${m.role}</small></td><td>${period===1?(m.self?'Autoevaluación ✓':'Autoevaluación pendiente'):(m.check2?'Reunión de feedback 2 ✓':'Reunión de feedback 2 pendiente')}</td><td>${period===1?(m.upward?'Evaluación a jefe ✓':'Evaluación a jefe pendiente'):(m.s1!=null?'Evaluación 1 ✓':'Evaluación 1 pendiente')}</td><td>${complete?status('Completada','success'):ready?status('Lista para evaluar','success'):status('Pendiente','warning')}</td><td>${ready?`<button class="btn small primary team-eval-start" data-member="${m.id}">Evaluar</button>`:complete?'<span class="small-muted">Finalizada</span>':'<button class="btn small secondary" disabled>No disponible</button>'}</td></tr>`}).join(''); const own=isDean?'':(()=>{const ownSelf=ownEvalDone('selfSubmitted',period),ownUp=ownEvalDone('upwardSubmitted',period);return `<div class="evaluation-cards"><article class="evaluation-card"><div class="evaluation-card-head"><h2>Mi autoevaluación</h2>${status(ownSelf?'Enviada':'Pendiente',ownSelf?'success':'warning')}</div><p>Completa tu propia evaluación cuando corresponda.</p><div class="evaluation-card-actions">${btn(ownSelf?'Consultar':'Comenzar','primary','data-start-eval="self"')}</div></article><article class="evaluation-card"><div class="evaluation-card-head"><h2>Evaluación a mi jefe</h2>${status(ownUp?'Enviada':'Pendiente',ownUp?'success':'warning')}</div><p>Completa la evaluación de tu jefatura.</p><div class="evaluation-card-actions">${btn(ownUp?'Consultar':'Comenzar','primary','data-start-eval="upward"')}</div></article></div>`})(); c.innerHTML=`${isDean?'<div class="info-callout"><strong>Rol Decano:</strong> participa como evaluador del equipo administrativo y no realiza autoevaluación.</div>':''}${own}<section class="panel table-panel" style="margin-top:18px"><div class="panel-heading"><div><h2>Evaluación ${period} · Mi equipo</h2><p>El botón Evaluar se habilita únicamente cuando la persona cumple los prerrequisitos de la etapa.</p></div><span class="status-pill neutral">${stageLabel()}</span></div><table><thead><tr><th>Colaborador</th><th>Prerrequisito 1</th><th>Prerrequisito 2</th><th>Estado</th><th>Acción</th></tr></thead><tbody>${rows}</tbody></table></section>`;
  }else{const ownSelf=ownEvalDone('selfSubmitted',period),ownUp=ownEvalDone('upwardSubmitted',period),both=ownSelf&&ownUp;c.innerHTML=`${both?'<div class="alert-banner"><div><strong>Actividades completadas</strong><p>Has finalizado las evaluaciones que te corresponden en esta etapa.</p></div></div>':''}<div class="evaluation-cards"><article class="evaluation-card"><div class="evaluation-card-head"><h2>Autoevaluación</h2>${status(ownSelf?'Enviada':'Pendiente',ownSelf?'success':'warning')}</div><p>Valora tu desempeño y registra las evidencias solicitadas.</p><div class="evaluation-card-actions">${btn(ownSelf?'Consultar':'Comenzar','primary','data-start-eval="self"')}</div></article><article class="evaluation-card"><div class="evaluation-card-head"><h2>Evaluación a mi jefe</h2>${status(ownUp?'Enviada':'Pendiente',ownUp?'success':'warning')}</div><p>Valora los comportamientos de liderazgo observados durante el período.</p><div class="evaluation-card-actions">${btn(ownUp?'Consultar':'Comenzar','primary','data-start-eval="upward"')}</div></article></div>`;}
  $$('[data-start-eval]',c).forEach(b=>b.addEventListener('click',()=>startEvaluation(b.dataset.startEval))); $$('.team-eval-start',c).forEach(b=>b.addEventListener('click',()=>startEvaluation('manager',b.dataset.member)));
}
...
function teamStageLabel(m){if(m.s2!=null)return'Ciclo completado';if(m.check2)return'Evaluación 2';if(m.s1!=null)return'Reunión de feedback 2';if(m.self&&m.upward)return'Listo para Evaluación 1';if(m.check1)return'Preparación Evaluación 1';if(m.start)return'Reunión de feedback 1';return'Sesión de inicio'}
...
function teamComponentLabel(key){return ({self:'Autoevaluación',manager:'Evaluación del jefe',upward:'Evaluación de subordinados',service:'Evaluación de servicio'})[key]||key}
function teamSchemeTitle(m){return `${m.hasSubordinates===false?'Sin subordinados':'Con subordinados'} · ${m.serviceEligible?'Con servicio':'Sin servicio'}`}
function teamPeriodBreakdown(m,period){
  const total=period===1?m.s1:m.s2;
  if(total===null||total===undefined||total==='')return null;
  const schemeKey=weightScenarioKey(m.hasSubordinates!==false,m.serviceEligible);
  const scheme=state.weightSchemes[schemeKey]||{};
  const components=applicableWeightComponents(schemeKey);
  const configured=components.length>0&&components.every(k=>isDefinedWeight(scheme[k]))&&components.reduce((sum,k)=>sum+(+scheme[k]||0),0)===100;
  if(!configured)return {total:+total,schemeKey,configured:false,components:[]};
  const sample=(m.resultComponents&&m.resultComponents[period===1?'e1':'e2'])||{};
  const rows=components.map(key=>{
    const score=isDefinedWeight(sample[key])?+sample[key]:+total;
    const weight=+scheme[key]||0;
    return {key,label:teamComponentLabel(key),score,weight,points:score*weight/100};
  });
  return {total:+total,schemeKey,configured:true,rows,components};
}
function teamPeriodDetailCard(m,period){
  const data=teamPeriodBreakdown(m,period);
  const title=`Evaluación ${period}`;
  if(!data)return `<section class="team-eval-breakdown pending"><div class="team-breakdown-head"><div><h3>${title}</h3><p>${period===1?'Primer':'Segundo'} período</p></div><strong>Pendiente</strong></div><div class="empty-inline">El detalle estará disponible cuando la evaluación se encuentre cerrada.</div></section>`;
  if(!data.configured)return `<section class="team-eval-breakdown"><div class="team-breakdown-head"><div><h3>${title}</h3><p>${period===1?'Primer':'Segundo'} período</p></div><strong>${data.total.toFixed(1)}/100</strong></div><div class="info-callout"><strong>Ponderación pendiente:</strong> el escenario aplicable todavía no tiene porcentajes aprobados por RRHH.</div></section>`;
  const sum=data.rows.reduce((a,r)=>a+r.points,0);
  return `<section class="team-eval-breakdown"><div class="team-breakdown-head"><div><h3>${title}</h3><p>${period===1?'Primer':'Segundo'} período · ${teamSchemeTitle(m)}</p></div><strong>${data.total.toFixed(1)}/100</strong></div><div class="team-component-table"><div class="team-component-row team-component-header"><span>Componente</span><span>Nota</span><span>Peso</span><span>Aporte</span></div>${data.rows.map(r=>`<div class="team-component-row"><span>${r.label}</span><strong>${r.score.toFixed(1)}/100</strong><span>${r.weight}%</span><strong>${r.points.toFixed(1)} pts</strong></div>`).join('')}<div class="team-component-row team-component-total"><strong>Total evaluación ${period}</strong><span></span><strong>100%</strong><strong>${sum.toFixed(1)}/100</strong></div></div></section>`;
}
function teamResultDetails(m){
  const final=teamFinalScore(m);
  const finalLabel=final===null?'Pendiente':`${final.toFixed(1)}/100`;
  const classification=final!==null?performanceLabel(final):'Pendiente';
  const kind=final===null?'neutral':final>=92?'success':final>=81?'warning':'danger';
  return `<details class="team-result-person"><summary><div class="team-result-person-main"><div class="mini-avatar">${m.initials}</div><div><strong>${m.name}</strong><small>${m.role} · ${m.area}</small></div></div><div class="team-result-overview"><div class="team-final-score"><small>Resultado final</small><strong>${finalLabel}</strong></div>${status(classification,kind)}</div></summary><div class="team-result-detail"><div class="team-period-summary"><article><span>Evaluación 1</span><strong>${m.s1!=null?m.s1.toFixed(1)+'/100':'Pendiente'}</strong><small>${m.s1!=null?'50% del resultado final':'Primer período'}</small></article><article><span>Evaluación 2</span><strong>${m.s2!=null?m.s2.toFixed(1)+'/100':'Pendiente'}</strong><small>${m.s2!=null?'50% del resultado final':'Segundo período'}</small></article><article class="team-final-card"><span>Resultado final</span><strong>${final!==null?final.toFixed(1)+'/100':'Pendiente'}</strong><small>${final!==null?`(${m.s1.toFixed(1)} + ${m.s2.toFixed(1)}) / 2`:'Se calcula al cerrar E1 y E2'}</small></article><article><span>Evaluación de servicio</span><strong>${m.serviceEligible?'Aplica':'No aplica'}</strong><small>${m.serviceEligible&&m.serviceScore?`Nota fuente: ${m.serviceScore}/100`:'Según configuración RRHH'}</small></article></div><div class="team-breakdown-grid">${teamPeriodDetailCard(m,1)}${teamPeriodDetailCard(m,2)}</div>${final!==null?`<div class="team-final-formula"><div><strong>Cálculo del resultado final</strong><span>Promedio simple de las dos evaluaciones, cada una con peso de 50%.</span></div><div class="team-final-equation">(${m.s1.toFixed(1)} + ${m.s2.toFixed(1)}) / 2 = <strong>${final.toFixed(1)}/100</strong></div></div>`:''}</div></details>`;
}
function renderTeam(){
  ...
  root.innerHTML=`<div class="summary-grid team-result-summary"><div class="summary-box"><span class="label">Colaboradores</span><strong>${state.team.length}</strong><small>Equipo directo</small></div><div class="summary-box"><span class="label">Resultados finales disponibles</span><strong>${completed.length}</strong><small>Con E1 y E2 cerradas</small></div><div class="summary-box"><span class="label">Promedio del equipo</span><strong>${avg!==null?avg.toFixed(1)+'%':'-'}</strong><small>Promedio de resultados finales</small></div></div><section class="panel team-results-panel"><div class="panel-heading"><div><h2>Calificaciones del equipo</h2><p>La vista inicial muestra únicamente el resultado final y su clasificación. Despliega a cada colaborador para revisar E1, E2, la ponderación de sus componentes y el cálculo del promedio final.</p></div></div><div class="team-result-list">${state.team.map(teamResultDetails).join('')}</div></section>`;
}
...
function openSessionBuilder(type,targetId){
  const m=targetId?member(targetId):null; const isStart=type==='Sesión de inicio';
  if(isStart){
    openModal(`${type}${m?' · '+m.name:''}`,`<div class="meeting-builder"><section class="form-section"><h3>1. Revisión del perfil</h3><div class="read-only-block"><strong>${m?.name||'Ricardo García Ortiz'}</strong><small>${m?.role||'Coordinador de Procesos y Automatización'} · Perfil disponible para revisión</small></div></section><section class="form-section"><h3>2. Observaciones generales</h3><textarea class="control" placeholder="Registra observaciones relevantes de la conversación..."></textarea></section><section class="form-section"><h3>3. Acuerdos / próximos pasos</h3><textarea class="control" placeholder="Registra acuerdos y próximos pasos..."></textarea></section></div><div class="modal-actions"><button class="btn secondary" id="cancelSession">Cancelar</button><button class="btn primary" id="saveSession" data-session-type="${type}" data-session-target="${targetId||''}">Guardar acta</button></div>`,true); return;
  }
  const previous=targetId?(state.stage==='check2'?`Evaluación 1 de ${m.name}: ${m.s1!=null?m.s1+'/100':'resultado disponible al cierre'}. Acuerdos de la reunión anterior disponibles para seguimiento.`:`Sesión de inicio de ${m.name}: se revisó el perfil y se acordaron prioridades y próximos pasos.`):(state.stage==='check2'?'Resultado de Evaluación 1 y acuerdos de la reunión anterior disponibles para seguimiento.':'Sesión de inicio completada; se retoman los acuerdos y próximos pasos registrados.');
  openModal(`${type}${m?' · '+m.name:''}`,`<div class="meeting-builder"><section class="form-section"><h3>1. Contexto</h3><p class="helper">Este contenido se precarga desde el hito anterior y puede editarse para completar el contexto de la conversación actual.</p><textarea class="control" id="feedbackContext">${escapeAttr(previous)}</textarea></section><section class="form-section"><h3>2. Obstáculos</h3><textarea class="control" placeholder="Registra barreras, dependencias o dificultades relevantes..."></textarea></section><section class="form-section"><h3>3. Acuerdos / próximos pasos</h3><textarea class="control" placeholder="Registra los compromisos y próximos pasos acordados..."></textarea></section></div><div class="modal-actions"><button class="btn secondary" id="cancelSession">Cancelar</button><button class="btn primary" id="saveSession" data-session-type="${type}" data-session-target="${targetId||''}">Guardar acta</button></div>`,true);
}
...
function unifiedResultHtml(model,model2){
  ...
  <section class="panel result-plan-panel compact-result-plan"><div class="panel-heading"><div><h2>Plan del período</h2><p>Acciones acordadas después de la Evaluación 1 y revisadas en el siguiente reunión de feedback.</p></div>${status('En seguimiento','neutral')}</div><div class="plan-inline"><ul class="clean-list"><li>Fortalecer automatización de reportes recurrentes.</li><li>Completar capacitación en gestión de indicadores.</li><li>Revisar avance en el siguiente reunión de feedback.</li></ul><button class="btn secondary open-calculation">Ver cómo se calcula</button></div></section>
  <section class="panel talent-unified-panel">
    ...
    <div class="timeline-vertical result-timeline">${resultTimelineItem('Agosto 2027','start','Sesión de inicio',state.cycle==='pilot'?'Perfil revisado y acuerdos definidos.':'Perfil revisado, objetivos/KPIs y acuerdos definidos.')}${resultTimelineItem('Octubre 2027','check1','Reunión de feedback 1','Avances, obstáculos y acuerdos documentados.')}${resultTimelineItem('Enero 2028','eval1',`Evaluación 1 · ${s1.toFixed(1)}/100`,'Resultado del primer período y plan de trabajo.')}${resultTimelineItem('Abril 2028','check2','Reunión de feedback 2','Seguimiento con referencia al resultado del primer período.')}${resultTimelineItem('Junio 2028','eval2',`Evaluación 2 · ${s2.toFixed(1)}/100`,'Cierre del ciclo y resultado final consolidado.')}</div>
  </section>`;
}

function renderResults(){
  const c=$('#resultsContent'); const model=resultModel('own'); const model2=secondPeriodModel(model); const s1=model.overall,s2Raw=model2.overall,s2=state.cycle==='pilot'&&stageIndex()<4?null:s2Raw,final=finalCycleScore(s1,s2);
  if(state.cycle==='pilot'){
    c.innerHTML=`<section class="panel pilot-results"><div class="panel-heading"><div><p class="section-kicker">Resultados del piloto</p><h2>${final!==null?'Resultado final · '+final.toFixed(1)+'/100':'Resultados del ciclo'}</h2><p>Vista simple para el piloto. El resultado final corresponde al promedio de Evaluación 1 y Evaluación 2 cuando ambas estén cerradas.</p></div>${final!==null?status(performanceLabel(final),final>=92?'success':final>=81?'warning':'danger'):status('En proceso','neutral')}</div><div class="pilot-result-grid"><article><span>Evaluación 1</span><strong>${s1!=null?s1.toFixed(1)+'/100':'-'}</strong><small>Primer período</small></article><article><span>Evaluación 2</span><strong>${s2!=null?s2.toFixed(1)+'/100':'-'}</strong><small>Segundo período</small></article><article class="pilot-final"><span>Resultado final</span><strong>${final!==null?final.toFixed(1)+'/100':'Pendiente'}</strong><small>${final!==null?'(E1 + E2) / 2':'Se calcula al cerrar ambas evaluaciones'}</small></article></div></section>`; return;
  }
  c.innerHTML=`<div class="results-tabs"><button class="tab active" data-results-tab="myResult">Mi resultado</button><button class="tab" data-results-tab="calculation">Cómo se calcula</button></div><div id="myResult" class="results-pane active">${unifiedResultHtml(model,model2)}</div><div id="calculation" class="results-pane">${detailedCalculationHtml(model,false)}</div>`; $$('[data-results-tab]',c).forEach(b=>b.addEventListener('click',()=>{$$('[data-results-tab]',c).forEach(x=>x.classList.toggle('active',x===b));$$('.results-pane',c).forEach(x=>x.classList.toggle('active',x.id===b.dataset.resultsTab));})); $$('.open-calculation',c).forEach(b=>b.addEventListener('click',()=>{$('[data-results-tab="calculation"]',c).click()})); $$('.result-milestone',c).forEach(b=>b.addEventListener('click',()=>openResultMilestone(b.dataset.milestone,model,model2)));
}
...
function renderAdmin(){
  $('#campaignStatus').textContent=state.campaignStarted?'Activo':'Borrador'; $('#campaignStatus').className=`status-pill ${state.campaignStarted?'success':'neutral'}`; $('#adminCycleType').value=state.cycle;
  ...
  $$('.post-pilot-tab,.post-pilot-pane').forEach(x=>x.style.display=state.cycle==='pilot'?'none':'');
  renderExceptions();renderQuestionAdmin();renderWeightSchemes();renderMatrix();renderTracking();
}

function matrixDataset(){
  const teamRows=state.team.map(m=>({id:m.id,name:m.name,role:m.role,area:m.area,manager:'Ricardo García Ortiz',managerType:'administrativo',selfApplicable:true,upwardApplicable:true,hasSubordinates:m.hasSubordinates!==false,subordinates:m.hasSubordinates===false?0:1,serviceEligible:m.serviceEligible,serviceScore:m.serviceScore,editableService:true}));
  return teamRows.concat([
    {id:'ricardo-matrix',name:'Ricardo García Ortiz',role:'Coordinador de Procesos y Automatización',area:'Mejoramiento Continuo',manager:'María Andrade',managerType:'administrativo',selfApplicable:true,upwardApplicable:true,hasSubordinates:true,subordinates:state.team.length,serviceEligible:false,editableService:false},
    {id:'paula-dean-team',name:'Paula Navas',role:'Coordinadora Administrativa',area:'Colegio Académico',manager:'Elena Viteri',managerType:'decano',selfApplicable:true,upwardApplicable:false,hasSubordinates:false,subordinates:0,serviceEligible:false,editableService:false}
  ]);
}
function downloadCsv(filename,headers,rows){const esc=v=>`"${String(v??'').replace(/"/g,'""')}"`;const csv='﻿'+[headers.map(esc).join(','),...rows.map(r=>r.map(esc).join(','))].join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download=filename;a.click();URL.revokeObjectURL(a.href);}
function trackingDataset(){
  return matrixDataset().filter(x=>x.id!=='ricardo-matrix').map(r=>{const m=member(r.id); if(!m)return {...r,currentStage:'Evaluación 1',processStatus:'En proceso',s1:null,s2:null,final:null}; let currentStage='Sesión de inicio',processStatus='No iniciado'; if(m.start){currentStage='Reunión de feedback 1';processStatus='En proceso'} if(m.check1){currentStage='Evaluación 1';processStatus=(m.self&&m.upward&&!m.manager)?'Listo para evaluar':'En proceso'} if(m.s1!=null){currentStage='Reunión de feedback 2';processStatus='En proceso'} if(m.check2){currentStage='Evaluación 2';processStatus=m.s2==null?'Listo para evaluar':'Completado'} if(m.s2!=null)processStatus='Completado'; return {...r,currentStage,processStatus,s1:m.s1,s2:m.s2,final:teamFinalScore(m)};});
}
function renderTracking(){
  const body=$('#trackingRows');if(!body)return; state.trackingFilters=state.trackingFilters||{person:'',status:'all',stage:'all'}; const f=state.trackingFilters,q=(f.person||'').toLowerCase(); const rows=trackingDataset().filter(r=>(!q||`${r.name} ${r.role}`.toLowerCase().includes(q))&&(f.status==='all'||r.processStatus===f.status)&&(f.stage==='all'||r.currentStage===f.stage)); body.innerHTML=rows.map(r=>`<tr><td><strong>${r.name}</strong><small>${r.role}</small></td><td>${r.manager}</td><td>${r.currentStage}</td><td>${status(r.processStatus,r.processStatus==='Completado'?'success':r.processStatus==='Listo para evaluar'?'success':r.processStatus==='No iniciado'?'warning':'neutral')}</td><td>${r.s1??'-'}</td><td>${r.s2??'-'}</td><td>${r.final!==null?r.final.toFixed(1)+'/100':'-'}</td></tr>`).join('')||'<tr><td colspan="7" class="empty-inline">No hay registros que coincidan con los filtros.</td></tr>';
}
function renderMatrix(){
  const body=$('#matrixRows');if(!body)return; state.matrixFilters=state.matrixFilters||{person:'',manager:'all'};
  const rows=matrixDataset(); const managers=[...new Set(rows.map(x=>x.manager))].sort((a,b)=>a.localeCompare(b,'es')); const managerSelect=$('#matrixManagerFilter'); if(managerSelect){managerSelect.innerHTML='<option value="all">Todos</option>'+managers.map(x=>`<option value="${escapeAttr(x)}">${x}</option>`).join(''); managerSelect.value=state.matrixFilters.manager||'all';}
  const q=(state.matrixFilters.person||'').toLowerCase(); const filtered=rows.filter(r=>(!q||`${r.name} ${r.role} ${r.area}`.toLowerCase().includes(q))&&((state.matrixFilters.manager||'all')==='all'||r.manager===state.matrixFilters.manager));
  body.innerHTML=filtered.map(r=>{const schemeKey=weightScenarioKey(r.hasSubordinates,r.serviceEligible),configured=state.weightSchemes[schemeKey]?.configured;return `<tr><td><strong>${r.name}</strong><small>${r.role} · ${r.area}</small></td><td>${r.manager}${r.managerType==='decano'?'<small>Decano · solo evaluador</small>':''}</td><td>${r.selfApplicable?'✓':'No aplica'}</td><td>${r.upwardApplicable?'✓':'No aplica'}</td><td>${r.subordinates}</td><td><label class="service-toggle-cell"><input class="service-checkbox service-edit" data-member="${r.id}" type="checkbox" ${r.serviceEligible?'checked':''} ${r.editableService?'':'disabled'}><span>${r.serviceEligible?'Aplica':'No aplica'}</span></label>${r.serviceEligible&&r.serviceScore?`<small>Nota servicio: ${r.serviceScore}/100 · fuente externa</small>`:''}</td><td><strong>${schemeTitle(schemeKey)}</strong>${configured?'':'<small class="pending-value">Requiere configuración 100%</small>'}</td><td>${status('Correcto','success')}</td></tr>`}).join('')||'<tr><td colspan="8" class="empty-inline">No hay registros que coincidan con los filtros.</td></tr>';
  $$('.service-edit',body).forEach(x=>x.addEventListener('change',e=>{const m=member(e.target.dataset.member); if(!m)return; m.serviceEligible=e.target.checked;saveState();renderMatrix();toast(`Evaluación de servicio ${m.serviceEligible?'activada':'desactivada'} para ${m.name}`)}));
}
...
function renderWeightSchemes(){
  const root=$('#weightScenarioTable'); if(!root)return; const keys=['withSubsNoService','withSubsService','noSubsNoService','noSubsService'];
  root.innerHTML=`<div class="table-panel"><table class="weight-scenario-table"><thead><tr><th>Escenario</th><th>Autoevaluación</th><th>Jefe</th><th>Subordinados</th><th>Servicio</th><th>Total</th><th>Estado</th><th></th></tr></thead><tbody>${keys.map(key=>{const sc=state.weightSchemes[key],applicable=applicableWeightComponents(key),defined=applicable.filter(c=>isDefinedWeight(sc[c])),total=defined.reduce((a,c)=>a+(+sc[c]),0),configured=defined.length===applicable.length&&total===100;sc.configured=configured;const cell=c=>applicable.includes(c)?(isDefinedWeight(sc[c])?`${(+sc[c]).toFixed(0)}%`:'<span class="pending-value">Configurar</span>'):'<span class="not-applicable">No aplica</span>';const totalLabel=configured?'100%':(defined.length?`${total.toFixed(0)}% <small>(configuración)</small>`:'<span class="pending-value">Configurar</span>');return `<tr><td><strong>${schemeTitle(key)}</strong><small>${key.startsWith('withSubs')?'Subordinados detectados en Nómina GP':'Sin subordinados detectados en Nómina GP'}</small></td><td>${cell('self')}</td><td>${cell('manager')}</td><td>${cell('upward')}</td><td>${cell('service')}</td><td><strong>${totalLabel}</strong></td><td>${configured?status('Configurado','success'):status('Requiere configuración','warning')}</td><td><button class="btn small secondary edit-weight-scheme" data-scheme="${key}">Editar</button></td></tr>`}).join('')}</tbody></table></div>`; $$('.edit-weight-scheme',root).forEach(b=>b.addEventListener('click',()=>openWeightScenarioEditor(b.dataset.scheme)));
}
function openWeightScenarioEditor(key){
  const sc=state.weightSchemes[key],app=applicableWeightComponents(key);
  const fields=app.map(c=>`<label>${componentLabel(c)} (%)<input class="control scenario-weight-input" data-component="${c}" type="number" min="0" max="100" step="1" value="${isDefinedWeight(sc[c])?+sc[c]:''}" placeholder="Ingresar porcentaje"></label>`).join('');
...
const processGuideData={
  start:{title:'1. Sesión de inicio',purpose:'Iniciar el ciclo revisando el perfil y registrando observaciones y acuerdos / próximos pasos. El piloto no incluye objetivos/KPIs.',colaborador:['Consulta su perfil y funciones.','Participa en la sesión de inicio.','Consulta el acta una vez registrada.'],jefe:['Abre Sesiones y selecciona al colaborador.','Registra observaciones y acuerdos / próximos pasos.'],decano:['Realiza la sesión de inicio con su equipo administrativo.','No realiza autoevaluación.'],admin:['Configura fechas y comunicaciones.','Valida población administrativa y relaciones provenientes de Nómina GP.']},
  check1:{title:'2. Reunión de feedback 1',purpose:'Registrar una conversación breve de seguimiento utilizando el contexto anterior.',colaborador:['Participa en la conversación.','Consulta el acta registrada.'],jefe:['Contexto precargado y editable.','Registra obstáculos y acuerdos / próximos pasos.'],decano:['Registra la misma conversación para su equipo administrativo.'],admin:['Monitorea el estado desde Seguimiento.']},
  eval1:{title:'3. Evaluación 1',purpose:'Completar autoevaluación, evaluación ascendente y evaluación jefe → colaborador con reglas de habilitación.',colaborador:['Completa Autoevaluación con evidencias obligatorias.','Completa Evaluación a su jefe.'],jefe:['La lista del equipo habilita “Evaluar” solo cuando se cumplen prerrequisitos.','Completa la evaluación jefe → colaborador desde Evaluaciones.'],decano:['Solo evalúa a su equipo administrativo.','No realiza autoevaluación ni evaluación a jefe.'],admin:['Monitorea avance por estado y etapa.','Administra instrumentos, servicio y ponderaciones.']},
  check2:{title:'4. Reunión de feedback 2',purpose:'Retomar el resultado y acuerdos anteriores para registrar contexto, obstáculos y próximos pasos.',colaborador:['Participa en la conversación y consulta el acta.'],jefe:['Contexto precargado desde el hito anterior y editable.','Registra obstáculos y acuerdos / próximos pasos.'],decano:['Aplica el mismo flujo con su equipo administrativo.'],admin:['Monitorea pendientes desde Seguimiento.']},
  eval2:{title:'5. Evaluación 2 y cierre',purpose:'Cerrar las evaluaciones, calcular el promedio final de E1 y E2 y habilitar reportes básicos del piloto.',colaborador:['Completa las evaluaciones habilitadas.','Consulta E1, E2 y resultado final simple.'],jefe:['Evalúa desde Evaluaciones.','Consulta resultados finales del equipo desde Mi equipo.'],decano:['Evalúa al equipo administrativo; no tiene autoevaluación propia.'],admin:['Valida integridad de resultados y descarga reportes.','El Perfil de Talento, Comité, Excepciones y Cómo se calcula quedan fuera del piloto.']}
};
function renderProcessGuide(){
  const c=$('#processGuideContent'),d=processGuideData[state.stage]||processGuideData.eval2; c.innerHTML=`<div class="page-heading"><div><p class="eyebrow">Vista del prototipo para TI</p><h1>Flujo funcional del piloto</h1><p class="lead">Selecciona una etapa y un rol para visualizar qué pantalla se habilita, qué información consulta y dónde realiza cada acción.</p></div>${status(stageLabel(),'neutral')}</div><section class="panel"><div class="process-guide-steps">${['start','check1','eval1','check2','eval2'].map((k,i)=>`<button class="process-guide-step ${state.stage===k?'active':''}" data-guide-stage="${k}"><span>${i+1}</span><strong>${stageLabel(k)}</strong></button>`).join('')}</div></section><section class="panel process-guide-detail"><div class="panel-heading"><div><h2>${d.title}</h2><p>${d.purpose}</p></div></div><div class="role-view-grid">${[['colaborador','Colaborador'],['jefe','Jefe / Evaluador'],['decano','Decano / Evaluador'],['admin','RRHH / Administrador']].map(([key,label])=>`<article class="role-view-card"><h3>${label}</h3><ul>${(d[key]||[]).map(x=>`<li>${x}</li>`).join('')}</ul><button class="btn small secondary simulate-role" data-role="${key}">Simular este rol</button></article>`).join('')}</div></section><section class="panel"><div class="panel-heading"><div><h2>Fuera del alcance del piloto</h2><p>Estas capacidades permanecen documentadas como evolución posterior, pero no se habilitan para acelerar el lanzamiento.</p></div></div><div class="postpilot-tags"><span>Excepciones de estructura</span><span>Comité de Talento</span><span>Perfil de Talento / actas integradas</span><span>Objetivos y KPIs</span><span>Cómo se calcula</span></div></section>`; $$('[data-guide-stage]',c).forEach(b=>b.addEventListener('click',()=>{state.stage=b.dataset.guideStage;$('#stageSelect').value=state.stage;saveState();applyRole();navigate('processGuide')})); $$('.simulate-role',c).forEach(b=>b.addEventListener('click',()=>{state.role=b.dataset.role;$('#roleSelect').value=state.role;saveState();applyRole();toast('Rol simulado actualizado')}));
}
...
$('#cycleSelect').addEventListener('change',e=>{state.cycle=e.target.value;if(state.cycle==='pilot'&&['kpis','committee'].includes(state.page))state.page=state.role==='admin'?'admin':'dashboard';if(state.cycle==='pilot'&&state.role==='comite'){state.role='admin';$('#roleSelect').value='admin';state.page='admin';}saveState();applyRole();toast('Escenario del ciclo actualizado')});
...
$('#newSessionBtn').addEventListener('click',()=>openSessionBuilder('Reunión de feedback 2',null));
...
$('#validateMatrix').addEventListener('click',()=>toast('Validación completada: población administrativa, jefaturas y configuración de servicio revisadas.'));
...
$('#matrixPersonFilter')?.addEventListener('input',e=>{state.matrixFilters.person=e.target.value;saveState();renderMatrix()});
$('#matrixManagerFilter')?.addEventListener('change',e=>{state.matrixFilters.manager=e.target.value;saveState();renderMatrix()});
$('#clearMatrixFilters')?.addEventListener('click',()=>{state.matrixFilters={person:'',manager:'all'};$('#matrixPersonFilter').value='';saveState();renderMatrix()});
$('#trackingPersonFilter')?.addEventListener('input',e=>{state.trackingFilters.person=e.target.value;saveState();renderTracking()});
$('#trackingStatusFilter')?.addEventListener('change',e=>{state.trackingFilters.status=e.target.value;saveState();renderTracking()});
$('#trackingStageFilter')?.addEventListener('change',e=>{state.trackingFilters.stage=e.target.value;saveState();renderTracking()});
$('#clearTrackingFilters')?.addEventListener('click',()=>{state.trackingFilters={person:'',status:'all',stage:'all'};['trackingPersonFilter'].forEach(id=>$('#'+id).value='');$('#trackingStatusFilter').value='all';$('#trackingStageFilter').value='all';saveState();renderTracking()});
$('#downloadMatrix')?.addEventListener('click',()=>{const f=state.matrixFilters||{person:'',manager:'all'},q=(f.person||'').toLowerCase();const rows=matrixDataset().filter(r=>(!q||`${r.name} ${r.role} ${r.area}`.toLowerCase().includes(q))&&(f.manager==='all'||r.manager===f.manager));downloadCsv('matriz_evaluaciones_piloto.csv',['Evaluado','Cargo','Área','Jefe evaluador','Autoevaluación','Evalúa a su jefe','Servicio'],rows.map(r=>[r.name,r.role,r.area,r.manager,r.selfApplicable?'Sí':'No',r.upwardApplicable?'Sí':'No',r.serviceEligible?'Aplica':'No aplica']))});
$('#downloadTracking')?.addEventListener('click',()=>{const f=state.trackingFilters||{person:'',status:'all',stage:'all'},q=(f.person||'').toLowerCase();const rows=trackingDataset().filter(r=>(!q||`${r.name} ${r.role}`.toLowerCase().includes(q))&&(f.status==='all'||r.processStatus===f.status)&&(f.stage==='all'||r.currentStage===f.stage));downloadCsv('seguimiento_piloto.csv',['Persona','Cargo','Jefe evaluador','Etapa actual','Estado','E1','E2','Resultado final'],rows.map(r=>[r.name,r.role,r.manager,r.currentStage,r.processStatus,r.s1??'',r.s2??'',r.final!==null?r.final.toFixed(1):'']))});
...
document.addEventListener('click',e=>{
  ...
  if(e.target?.id==='saveKpiModal'){state.kpis.push({id:Date.now(),title:$('#mGoal').value||'Nuevo objetivo',indicator:$('#mIndicator').value||'Sin especificar',meta:$('#mMeta').value||'Sin especificar',base:$('#mBase').value||'Sin especificar',progress:0});saveState();renderKpis();closeModal();toast('Objetivo agregado')}
  if(e.target?.id==='saveSession'){const target=e.target.dataset.sessionTarget,type=e.target.dataset.sessionType;if(target){const m=member(target);if(type==='Sesión de inicio'){m.start=true;if(state.cycle!=='pilot')m.kpis=Math.max(m.kpis,2)}else if(type==='Reunión de feedback 2')m.check2=true;else m.check1=true;}else{state.sessions.push({id:Date.now(),date:new Date().toLocaleDateString('es-EC'),type,summary:type==='Sesión de inicio'?(state.cycle==='pilot'?'Perfil y acuerdos.':'Perfil, objetivos/KPIs y acuerdos.'):'Contexto, obstáculos y acuerdos / próximos pasos.',status:'Completada',participants:'Ricardo / María',agreements:['Acuerdo registrado desde el prototipo.'],next:['Próximo paso registrado desde el prototipo.']});}saveState();closeModal();toast('Acta guardada');if(target){renderTeamMember()}else renderSessions();}
