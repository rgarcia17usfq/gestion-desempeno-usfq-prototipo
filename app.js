...
const STORAGE='usfq-gd-v031-state';
...
function renderResults(){
  const c=$('#resultsContent');const isBoss=state.role==='jefe';const model=resultModel('own');
  c.innerHTML=`<div class="results-tabs"><button class="tab active" data-results-tab="myResult">Mi resultado</button><button class="tab" data-results-tab="calculation">Cómo se calcula</button><button class="tab" data-results-tab="annualCompare">Comparación anual</button><button class="tab" data-results-tab="talentProfile">Perfil de talento</button>${isBoss?'<button class="tab" data-results-tab="teamResults">Resultados de mi equipo</button>':''}</div>
  <div id="myResult" class="results-pane active"><div class="comparison-grid">${resultSummaryHtml(model)}<section class="panel"><div class="panel-heading"><div><h2>Plan del período</h2><p>Acciones acordadas después del resultado.</p></div></div><ul class="clean-list"><li>Fortalecer automatización de reportes recurrentes.</li><li>Completar capacitación en gestión de indicadores.</li><li>Revisar avance en el siguiente check-in.</li></ul><button class="btn secondary open-calculation">Ver desglose completo</button></section></div><div class="panel component-preview"><div class="panel-heading"><div><h2>Desagregación del puntaje</h2><p>Cómo cada evaluación aporta a la nota general.</p></div><strong class="overall-mini">${model.overall.toFixed(1)}/100</strong></div><div class="table-panel"><table><thead><tr><th>Componente</th><th>Nota /100</th><th>Peso</th><th>Aporte final</th></tr></thead><tbody>${componentSummaryRows(model)}</tbody></table></div></div></div>
  <div id="calculation" class="results-pane">${detailedCalculationHtml(model)}</div>
  <div id="annualCompare" class="results-pane"><div class="panel"><div class="panel-heading"><div><h2>Comparación Evaluación 1 vs Evaluación 2</h2><p>Vista disponible al cierre anual para identificar evolución.</p></div></div><div class="compare-bars"><div class="compare-row"><strong>Evaluación 1</strong><div class="compare-track"><span style="width:${Math.round(model.overall)}%"></span></div><strong>${model.overall.toFixed(1)}</strong></div><div class="compare-row"><strong>Evaluación 2</strong><div class="compare-track"><span style="width:92%"></span></div><strong>92.0</strong></div></div><div class="info-callout"><strong>Resultado anual:</strong> la fórmula exacta para consolidar S1 y S2 permanece pendiente de definición por RRHH; esta vista compara los resultados sin asumir una fórmula.</div></div></div>
  <div id="talentProfile" class="results-pane">${talentProfileHtml('Ricardo García Ortiz','RG','Coordinador de Procesos y Automatización','Mejoramiento Continuo',Math.round(model.overall),92)}</div>
  ${isBoss?`<div id="teamResults" class="results-pane"><div class="panel table-panel"><table><thead><tr><th>Colaborador</th><th>Evaluación 1</th><th>Evaluación 2</th><th>Plan</th><th></th></tr></thead><tbody>${state.team.map(m=>{const rm=resultModel(m.id);return `<tr><td><strong>${m.name}</strong><small>${m.role}${m.serviceEligible?' · con evaluación de servicio':''}</small></td><td>${m.manager?rm.overall.toFixed(1)+'/100':'Pendiente'}</td><td>${m.s2?m.s2+'/100':'-'}</td><td>${m.plan}</td><td>${m.manager?`<button class="link-btn team-result-open" data-member="${m.id}">Ver cálculo</button>`:'Resultado no disponible'}</td></tr>`}).join('')}</tbody></table></div></div>`:''}`;
  ...
  $('.open-calculation',c)?.addEventListener('click',()=>{$('[data-results-tab="calculation"]',c).click()});
